...
int main(int argc, char **argv)
{
  ...
  bool clockControl = true;
  n.getParam(ros::this_node::getName() + "/uavName", uavName);
  n.getParam(ros::this_node::getName() + "/clock_control", clockControl);
  
  // Init class
  Uav_Dynamics uav_dynamics_node(n, uavName, clockControl);
 ...
 */
Uav_Dynamics::Uav_Dynamics(ros::NodeHandle nh, std::string uavNameIn, bool clockControlIn):
// Node handle
node_(nh),
// TF listener
...
initPose_(7,0)
{
  ...
  uavName = uavNameIn;
  clockControl = clockControlIn;

  imu_.reset(new Uav_Imu(uavName));
  pid_.reset(new Uav_Pid(uavName));
  lpf_.reset(new Uav_LowPassFilter(uavName));

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/vehicle_mass", vehicleMass_)) { 
      std::cout << "Did not get the vehicle mass from the params, defaulting to 1kg" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/motor_time_constant", motorTimeconstant_)) { 
      std::cout << "Did not get the motor time constant from the params, defaulting to 0.02" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/moment_arm", momentArm_)) { 
      std::cout << "Did not get the moment arm from the params, defaulting to 0.08" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/thrust_coefficient", thrustCoeff_)) { 
      std::cout << "Did not get the thrust coefficient from the params, defaulting to 1.91e-6" << std::endl;
  }
  
  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/torque_coefficient", torqueCoeff_)) { 
      std::cout << "Did not get the torque coefficient from the params, defaulting to 2.6e-7" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/drag_coefficient", dragCoeff_)) { 
      std::cout << "Did not get the drag coefficient from the params, defaulting to 0.1" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/vehicle_inertia_xx", vehicleInertia_[0])) { 
      std::cout << "Did not get the inertia (x) from the params, defaulting to 0.0049 kg m^2" << std::endl;
  }
  
  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/vehicle_inertia_yy", vehicleInertia_[1])) { 
      std::cout << "Did not get the inertia (y) from the params, defaulting to 0.0049 kg m^2" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/vehicle_inertia_zz", vehicleInertia_[2])) { 
      std::cout << "Did not get the inertia (z) from the params, defaulting to 0.0069 kg m^2" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/max_prop_speed", maxPropSpeed_)) { 
      std::cout << "Did not get the max prop speed from the params, defaulting to 2200 rad/s" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/angular_process_noise", angAccelProcessNoiseAutoCorrelation_)) { 
      std::cout << "Did not get the angular process noise from the params, defaulting to 0.00025 rad^2/s^2" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/linear_process_noise", linAccelProcessNoiseAutoCorrelation_)) { 
      std::cout << "Did not get the linear process noise from the params, defaulting to 0.0005 m^2/s^3" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/reset_timeout", resetTimeout_)) {
      std::cout << "Did not value for reset timeout from the params, defaulting to 0.1 sec" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/min_arming_thrust", minArmingThrust_)) {
    std::cout << "Did not value for minimum arming thrust from the params, defaulting to 9.9 N" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/clockscale", clockScale)) {
    std::cout << "Using sim_time and did not get a clock scaling value. Defaulting to 1." << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_uav_dynamics/init_pose", initPose_)) {
...
//  initialPose.position.y = 2.0f;
  ...
  imuPub_ = node_.advertise<sensor_msgs::Imu>(uavName + "/sensors/imu", 96);  
  gpsPub_ = node_.advertise<geometry_msgs::PoseStamped>(uavName + "/sensors/gps", 96);
  velPub_ = node_.advertise<geometry_msgs::TwistStamped>(uavName + "/sensors/velocity", 96);
  armPub_ = node_.advertise<std_msgs::Bool>(uavName + "/armed", 96);
  
  inputCommandSub_ = node_.subscribe(uavName + "/input/rateThrust", 1, &Uav_Dynamics::inputCallback, this);

  tmp_time.clock = currentTime_;

  if (clockControl == true)
  {
    clockPub_ = node_.advertise<rosgraph_msgs::Clock>("/clock",1);
    clockPub_.publish(tmp_time);
  }
...
void Uav_Dynamics::publishState(void){
  ...
  transform.child_frame_id = uavName + "/imu";
 ...
 */
Uav_Imu::Uav_Imu(std::string uavNameIn){

  uavName = uavNameIn;

  if (!ros::param::get(uavName + "/flightgoggles_imu/accelerometer_variance", accMeasNoiseVariance_)) { 
      std::cout << "Did not get the accelerometer variance from the params, defaulting to " << accMeasNoiseVariance_ << " m^2/s^4" << std::endl;
  }

  if (!ros::param::get(uavName + "/flightgoggles_imu/gyroscope_variance", gyroMeasNoiseVariance_)) { 
 ...
 */
Uav_LowPassFilter::Uav_LowPassFilter(std::string uavNameIn){
  ...
  if ((!ros::param::get(uavName + "/flightgoggles_lpf/gain_p", temp1)) ||
      (!ros::param::get(uavName + "/flightgoggles_lpf/gain_q", temp2)))
  { 
