...
class Uav_Dynamics {
    public:
        /// @name Constructor
        // Requires nodehandle
        Uav_Dynamics(ros::NodeHandle nh, std::string uavNameIn, bool clockControlIn);
    ...
    private:
        ...
        void resetState(void);
        void publishState(void);        // Get the param name

        std::string uavName;
        bool clockControl;

        std::unique_ptr<Uav_Imu> imu_;
        std::unique_ptr<Uav_Pid> pid_;
        std::unique_ptr<Uav_LowPassFilter> lpf_;
