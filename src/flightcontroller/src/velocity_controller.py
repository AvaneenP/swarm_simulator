...
class VelocityController():
  ...
  def __init__(self):
    ...
    gains = rospy.get_param(str(rospy.get_name()) + '/gains', {'p_xy': 1, 'i_xy': 0.0, 'd_xy': 0.0, 'p_z': 1, 'i_z': 0.0, 'd_z': 0.0})
    ...
    self.vel_read_sub = rospy.Subscriber(self.uavName + "/sensors/velocity", TwistStamped, self.get_vel)
    self.vel_set_sub = rospy.Subscriber(self.uavName + "/input/velocity", Vector3 , self.set_vel)
    self.sub = rospy.Subscriber(self.uavName + "/sensors/attitude", Vector3, self.euler_angle_callback)

    # Create the publishers 
    self.att_pub = rospy.Publisher(self.uavName + "/input/attitude", Vector3, queue_size=1)
    self.thrust_pub = rospy.Publisher(self.uavName + "/input/thrust", Float64, queue_size=1)
