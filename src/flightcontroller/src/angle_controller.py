...
class AngleController():
  ...
  def __init__(self):
    ...
    gains = rospy.get_param(str(rospy.get_name()) + '/gains', {'p': 0.1, 'i': 0, 'd': 0})
    ...
    # Declare the uva name
    self.uavName = rospy.get_param(str(rospy.get_name()) + '/uavName', "uav")

    # Create the subscribers
    self.imu_sub = rospy.Subscriber(self.uavName + "/sensors/attitude", Vector3, self.euler_angle_callback)
    self.att_sub = rospy.Subscriber(self.uavName + "/input/attitude", Vector3, self.attitude_set_callback)
    self.thrust_sub = rospy.Subscriber(self.uavName + "/input/thrust", Float64, self.thrust_callback)
    self.armed_sub = rospy.Subscriber(self.uavName + "/armed", Bool, self.armed_callback)
    self.yaw_sub = rospy.Subscriber(self.uavName + "/input/yaw", Float64, self.set_yaw_output)

    # Create the publishers
    self.rate_pub = rospy.Publisher(self.uavName + "/input/rateThrust", RateThrust, queue_size=10)
