...
class PositionController():
  ...
  def __init__(self):
    ...
    stable_gains = rospy.get_param(str(rospy.get_name()) + '/gains/stable/', {'p': 1, 'i': 0.0, 'd': 0.0})
    ...
    self.gps_sub = rospy.Subscriber(self.uavName + "/sensors/gps", PoseStamped, self.get_gps)
    self.pos_set_sub = rospy.Subscriber(self.uavName + "/input/position", Vector3, self.set_pos)

    # Create the publishers
    self.vel_set_sub = rospy.Publisher(self.uavName + "/input/velocity", Vector3, queue_size=1)
